...
func (h *PointsHandler) TransferPoints(w http.ResponseWriter, r *http.Request) {
	...
	// VULN #3: No negative amount validation — allows stealing points
	if req.ReceiverIdentifier == "" {
		http.Error(w, `{"error":"receiver_identifier is required"}`, http.StatusBadRequest)
		return
	}
