...
func (r *PointsRepository) TransferPoints(senderID, receiverID uint64, amount int64, note string) (*model.PointTransaction, error) {
	// VULN #10: Race condition — balance checked OUTSIDE the transaction with NO row lock
	var senderBalance int64
	err := r.db.QueryRow("SELECT points_balance FROM users WHERE id = ?", senderID).Scan(&senderBalance)
	if err != nil {
	...
	// Deduct from sender (no re-check of balance inside tx)
