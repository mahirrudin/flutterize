...
func (r *UserRepository) FindByEmailOrPhone(identifier string) (*model.User, error) {
	user := &model.User{}
	query := `SELECT id, email, phone, fullname, birthdate, password, points_balance, created_at FROM users WHERE email = ? OR phone = ?`
	var birthdate string
	err := r.db.QueryRow(query, identifier, identifier).Scan(
		&user.ID, &user.Email, &user.Phone, &user.Fullname,
		&birthdate, &user.Password, &user.PointsBalance, &user.CreatedAt,
	)
	if err != nil {
		return nil, err
	}
...
func (r *UserRepository) FindByID(id uint64) (*model.User, error) {
	user := &model.User{}
	query := `SELECT id, email, phone, fullname, birthdate, password, points_balance, created_at FROM users WHERE id = ?`
	var birthdate string
...
func (r *UserRepository) FindByResetToken(token string) (*model.User, error) {
	user := &model.User{}
	query := `SELECT id, email, phone, fullname, birthdate, password, points_balance, created_at
...
// VULN #4: Mass assignment — accepts points_balance from request
func (r *UserRepository) Create(req model.RegisterRequest, hashedPassword string) (*model.User, error) {
	// Points_balance comes from the request body — attacker can set arbitrary points
	points := req.PointsBalance
	result, err := r.db.Exec(
		`INSERT INTO users (email, phone, fullname, birthdate, password, points_balance) VALUES (?, ?, ?, ?, ?, ?)`,
		req.Email, req.Phone, req.Fullname, req.Birthdate, hashedPassword, points,
	)
	if err != nil {
		return nil, err
	}
